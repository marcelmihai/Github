...
class EventHandler(PersistentServerConnectionApplication):
    ...
    def isDataFieldJSONObject(self, event):
        if type(event['data']) is not dict:
            return False
        return True

    def isComponentFieldString(self, event):
        if type(event['component']) is not unicode:
            return False
        return True
    ...
    def bundleDto(self, events):
        dto = []
        for event in events:
            if not self.isDataFieldJSONObject(event):
                raise Exception(["Data field must be a JSON Object"])

            if not self.isComponentFieldString(event):
                raise Exception(["Component field must be a string"])
