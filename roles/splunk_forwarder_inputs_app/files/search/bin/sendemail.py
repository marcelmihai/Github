                        ...
                        <td style="text-align: left; padding: 4px 8px; margin-top: 0px; margin-bottom: 0px; border-bottom: 1px dotted #ccc;">
                            % if isinstance(result.get(col), list):
                                % for val in result.get(col):
                                    <div style="white-space: pre-wrap;">${val|h}</div>
                                % endfor
                            % else:
                                <div style="white-space: pre-wrap;">${result.get(col)|h}</div>
                            % endif
                        </td>
                    % endfor
                    </tr>
...
% if len(results) > 0:
    % if results[0].get("_raw"):
        ...
        % for result in results:
            <div class="event"style="border-bottom: 1px dotted #ccc; padding: 5px 0; font-family: monospace; word-break: break-all;"><pre style="white-space: pre-wrap;">${result.get("_raw", "")|h}</pre></div>
        % endfor
        </div>
    % else:
