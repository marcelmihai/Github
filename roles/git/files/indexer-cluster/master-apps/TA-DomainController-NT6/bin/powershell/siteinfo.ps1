...
write-host  $Date Type=`"Site`" Site=`"$SiteName`" Location=`"$($SiteInfoObj.Location)`" -NoNewline
...
$SiteInfoObj.Subnets | Foreach-Object {
    write-Host $Date Type=`"Subnet`" Name=`"$($_.Name)`" Site=`"$SiteName`" Location=`"$($_.Location)`"
}	
